...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Flow Deletion Tool")
        self.geometry("1000x600")  # Increase the window width for better readability
    ...
    def create_widgets(self):
        ...
        self.process_button = ttk.Button(action_frame, text="Process Flows", command=self.process_flow, state=tk.DISABLED)
        ...
        self.delete_inactive_button.grid(row=0, column=1, padx=5, pady=5)

        self.query_flows_button = ttk.Button(action_frame, text="Query All Flows", command=self.query_all_flows, state=tk.DISABLED)
        self.query_flows_button.grid(row=0, column=2, padx=5, pady=5)
    ...
    def delete_flow(self, flow_id):
        ...
        if response.status_code == 400 and "DELETE_FAILED" in response.text:
            self.text_area.insert(tk.END, Fore.YELLOW + f"Skipping deletion of active flow version with ID '{flow_id}'.\n")
            self.scroll_to_bottom()
        else:
            response.raise_for_status()
            self.text_area.insert(tk.END, Fore.GREEN + f"Flow with ID '{flow_id}' deleted successfully.\n")
            self.scroll_to_bottom()

    def display_flow_definition_info(self, flow_definition_info):
        self.text_area.insert(tk.END, Fore.YELLOW + "FlowDefinition Details:\n")
        table = PrettyTable()
    ...
    def display_flow_versions(self, flow_versions):
        self.text_area.insert(tk.END, Fore.YELLOW + "\nFlow Versions:\n")
        table = PrettyTable()
    ...
    def get_user_selection(self, flow_versions):
        self.text_area.insert(tk.END, Fore.YELLOW + "Available Flow Versions:\n")
        table = PrettyTable()
        table.field_names = ["Index", "ID", "Version Number", "API Version", "Definition ID"]
        for index, fv in enumerate(flow_versions, start=1):
            table.add_row([index, fv['Id'], fv['VersionNumber'], fv['ApiVersion'], fv['DefinitionId']])
        self.text_area.insert(tk.END, table.get_string() + "\n")
        self.text_area.insert(tk.END, Fore.YELLOW + "Select the versions you want to delete (comma-separated indexes, e.g., '1,3,5' or 'all' for all versions): \n")
        self.scroll_to_bottom()
    ...
    def get_user_selection_for_flows(self, all_flows):
        self.text_area.insert(tk.END, Fore.YELLOW + "Select the flows you want to process by entering the index numbers (comma-separated, e.g., '1,3,5'): \n")
        self.scroll_to_bottom()
        user_input = self.get_user_input()
        selected_indexes = [int(index.strip()) for index in user_input.split(',') if index.strip().isdigit()]
    ...
    def delete_inactive_versions(self):
        ...
        if all_flows:
            ...
            if confirmation:
                ...
                self.text_area.insert(tk.END, Fore.RED + "Deletion cancelled.\n")
                self.scroll_to_bottom()
        else:
            self.text_area.insert(tk.END, Fore.RED + "No active flows found in the org.\n")
            self.scroll_to_bottom()
    ...
    def delete_inactive_versions_for_flow(self, flow_definition_info):
        ...
        if inactive_versions:
            self.text_area.insert(tk.END, Fore.YELLOW + f"Deleting inactive versions for flow: {flow_definition_info['DeveloperName']}\n")
            self.scroll_to_bottom()
            for fv in inactive_versions:
                self.delete_flow(fv['Id'])
            self.text_area.insert(tk.END, Fore.GREEN + f"Inactive versions deleted successfully for flow: {flow_definition_info['DeveloperName']}\n")
            self.scroll_to_bottom()
        else:
            self.text_area.insert(tk.END, Fore.YELLOW + f"No inactive versions found for flow: {flow_definition_info['DeveloperName']}\n")
            self.scroll_to_bottom()

    def process_flow(self):
        if self.config:
            flow_api_names = [flow_api_name.strip() for flow_api_name in self.config.get('Salesforce', 'flow_api_names').split(',')]
                    ...
                    if flow_versions:
                        ...
                        if selected_ids:
                            ...
                            if confirmation:
                                ...
                                self.text_area.insert(tk.END, Fore.GREEN + "Selected Flow versions deleted successfully!\n")
                                self.scroll_to_bottom()
                            else:
                                self.text_area.insert(tk.END, Fore.RED + "Deletion cancelled.\n")
                                self.scroll_to_bottom()
                        else:
                            self.text_area.insert(tk.END, Fore.RED + "No Flow versions selected.\n")
                            self.scroll_to_bottom()
                    else:
                        self.text_area.insert(tk.END, Fore.RED + "No Flow versions found.\n")
                        self.scroll_to_bottom()
                else:
                    self.text_area.insert(tk.END, Fore.RED + f"No FlowDefinition found for {flow_api_name}.\n")
                    self.scroll_to_bottom()
        else:
        ...
        if self.config:
            ...
            if all_flows:
                ...
                if selected_flow_api_names:
                    self.text_area.insert(tk.END, Fore.GREEN + f"Selected flows for processing: {', '.join(selected_flow_api_names)}\n")
                    self.scroll_to_bottom()
                    for flow_name in selected_flow_api_names:
                        self.process_selected_flow(flow_name)
                else:
                    self.text_area.insert(tk.END, Fore.RED + "No flows selected for processing.\n")
                    self.scroll_to_bottom()
            else:
                self.text_area.insert(tk.END, Fore.RED + "No flows found to display.\n")
                self.scroll_to_bottom()
        else:
            messagebox.showerror("Error", "Please load a config file first.")
    ...
    def process_selected_flow(self, flow_api_name):
        ...
        if flow_definition_info:
            ...
            if flow_versions:
                ...
                self.text_area.insert(tk.END, Fore.RED + "No flow versions found for this flow.\n")
                self.scroll_to_bottom()
        else:
            self.text_area.insert(tk.END, Fore.RED + f"No FlowDefinition found for {flow_api_name}.\n")
            self.scroll_to_bottom()
